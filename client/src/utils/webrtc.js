...
class WebRTCService {
  ...
  async _setupLocalStream(video = true, audio = true) {
    ...
    try {
      ...
      this.localStream.getTracks().forEach((track) => {
        track.enabled = true;
        track.muted = false; // Ensure track is not muted
        console.log(`Track ${track.kind} enabled:`, track.enabled);
      });
  ...
  _createPeerConnection(userId, forceRelay = false) {
    ...
    pc.ontrack = (event) => {
      ...
      if (stream) {
        ...
        // Double-check all audio tracks in the stream
        stream.getAudioTracks().forEach(track => {
          track.enabled = true;
          if (track.muted) {
            console.log(`🔊 Unmuting audio track in stream for ${userId}`);
            track.muted = false;
          }
        });
  ...
  toggleAudio(enabled) {
    if (this.localStream) {
      this.localStream.getAudioTracks().forEach((track) => {
        track.enabled = enabled;
        if (enabled) {
          track.muted = false; // Ensure track is unmuted when enabled
        }
      });
      console.log(`🔊 Audio ${enabled ? "enabled" : "disabled"}`);
    }
