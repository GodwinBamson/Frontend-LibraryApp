...
class WebRTCService {
  constructor() {
    ...
    this.iceCandidateQueue = new Map(); // Queue for ICE candidates
  ...
  _setupListeners() {
    ...
    this.socket.on("user-joined", async (data) => {
      ...
      if (data.userId !== this.currentUser?.userId) {
        // Always create an offer when someone joins, regardless of userId comparison
        // This ensures both sides try to connect
      ...
      if (data.participants) {
        ...
        participants.forEach((p) => {
          ...
          if (targetUserId && targetUserId !== this.currentUser?.userId) {
            ...
            console.log("Creating offer for room participant:", targetUserId);
            setTimeout(() => this._createOffer(targetUserId), 1500);
          }
        });
      }
    ...
    if (pc.signalingState === "have-local-offer") {
      pc.setRemoteDescription(new RTCSessionDescription(answer))
        .then(() => {
          ...
          const queuedCandidates = this.iceCandidateQueue.get(userId) || [];
  ...
  _handleIceCandidate(userId, candidate) {
    ...
    if (!pc) {
      console.log(`No peer connection for ${userId}, queueing ICE candidate`);
    ...
    if (!pc.remoteDescription) {
      console.log(`Remote description not set yet, queueing ICE candidate for ${userId}`);
      if (!this.iceCandidateQueue.has(userId)) {
        this.iceCandidateQueue.set(userId, []);
      }
      this.iceCandidateQueue.get(userId).push(candidate);
      ...
      try {
        ...
        if (callType === "video") {
          ...
          if (!videoTracks || videoTracks.length === 0) {
            console.warn("No video track available, falling back to audio");
            callType = "audio";
  ...
  async _handleOffer(fromUserId, offer) {
    try {
      ...
      // Ensure local stream exists
      if (!this.localStream) {
        console.log(`No local stream, creating one before handling offer`);
        await this._setupLocalStream(this.currentCallType === "video", true);
      }
