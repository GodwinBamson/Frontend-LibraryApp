...
//     this.relayAttempted = new Set(); // Track relay attempts per user
//     this.connectionRetryCount = new Map(); // Track retry counts per user
...
//     // Configuration with multiple TURN servers
//     this.config = {
//       iceServers: [
//         // STUN servers for direct connection
...
//         // Open relay TURN (public, but limited)
//         {
//           urls: [
...
//           const candidateType = candidateStr.split(" ")[7];
//           console.log("TURN test candidate:", candidateType, candidateStr);
...
//     setTimeout(() => {
//       this.testTurnServers().then((relayWorking) => {
//         console.log(
//           "TURN servers test result:",
//           relayWorking ? "Working" : "Not working",
//         );
//       });
//     }, 2000);
...
//       console.log(
//         `Processing ${pendingBySocket.length} pending candidates for socket ${socketId} -> user ${userId}`,
//       );
...
//       console.log(
//         `Processing ${pendingByUser.length} pending candidates for user ${userId}`,
//       );
...
//   // Retry connection with relay only
//   async retryWithRelay(userId) {
//     if (this.relayAttempted.has(userId)) {
//       console.log(`Relay already attempted for ${userId}, skipping`);
//       return;
//     }

//     console.log(`🔄 Retrying connection with ${userId} using relay only`);
//     this.relayAttempted.add(userId);

//     // Increment retry count
//     const retryCount = (this.connectionRetryCount.get(userId) || 0) + 1;
//     this.connectionRetryCount.set(userId, retryCount);

//     if (retryCount > 3) {
//       console.log(`Max retries (3) reached for ${userId}, giving up`);
//       return;
//     }
...
//     const pc = this._createPeerConnection(userId, true);

//     // Re-add tracks
//     if (this.localStream) {
//       this.localStream.getTracks().forEach((track) => {
//         console.log(
//           `Adding ${track.kind} track to relay connection for ${userId}`,
//         );
...
//     // Re-create offer
//     try {
//       console.log(`Creating relay offer for ${userId}`);
...
//       console.log(`📞 Sending relay offer to user:`, userId);
...
//       });
//     } catch (error) {
//       console.error("Error retrying with relay:", error);
//     }
//   }
...
//       this.relayAttempted.delete(userId);
...
//     this.socket.on(
//       "ice-candidate",
//       ({ from, fromUserId, fromUsername, candidate }) => {
...
//         if (!targetUserId) {
//           console.log(
//             `Could not determine userId for ICE candidate from ${from}, storing by socketId`,
//           );
...
//             this.pendingIceCandidatesByUserId.get(userId) || [];
...
//     // Reset relay tracking for this user
//     this.relayAttempted.delete(targetUserId);
//     this.connectionRetryCount.delete(targetUserId);
...
//   _createPeerConnection(userId, forceRelay = false) {
//     if (this.peerConnections.has(userId)) {
//       this._cleanupPeerConnection(userId);
//     }

//     console.log(
//       `Creating peer connection for ${userId}${forceRelay ? " (forcing relay)" : ""}`,
//     );
...
//     } else {
//       console.warn(
//         `No local stream available for ${userId}, adding receive-only transceivers`,
//       );
...
//         console.log(
//           `❄️ Sending ICE candidate for ${userId} [Type: ${candidateType}]`,
//         );
...
//       console.error(`❄️ ICE candidate error for ${userId}:`, {
//         errorCode: event.errorCode,
//         errorText: event.errorText,
//         url: event.url,
//       });
...
//           this.connectionInProgress.delete(userId);
//           // Reset relay flag on successful connection
//           this.relayAttempted.delete(userId);
//           this.connectionRetryCount.delete(userId);
//           break;

//         case "failed":
//           console.log(`❌ ICE failed with ${userId}`);

//           // Check if we've already tried relay and haven't exceeded max retries
//           const retryCount = this.connectionRetryCount.get(userId) || 0;
//           if (!this.relayAttempted.has(userId) && retryCount < 3) {
//             console.log(
//               `Attempting relay connection for ${userId} (retry ${retryCount + 1}/3)`,
//             );
//             setTimeout(() => this.retryWithRelay(userId), 1000);
//           } else if (retryCount >= 3) {
//             console.log(`Max retries (3) reached for ${userId}, giving up`);
//             this.onConnectionStateChange?.({
//               state: "error",
//               data: `Failed to establish connection with ${userId} after multiple attempts`,
//             });
//           }
//           break;

//         case "disconnected":
//           console.log(`⚠️ ICE disconnected with ${userId}`);
//           // Give it a chance to recover
//           setTimeout(() => {
//             if (
//               pc.iceConnectionState === "disconnected" &&
//               !this.relayAttempted.has(userId) &&
//               (this.connectionRetryCount.get(userId) || 0) < 3
//             ) {
//               console.log(
//                 `Still disconnected, attempting ICE restart for ${userId}`,
//               );
//               pc.restartIce();
//             }
//           }, 5000);
...
//         console.log(
//           `   Audio track enabled: ${event.track.enabled}, readyState: ${event.track.readyState}`,
//         );

//         // Log audio track settings
//         console.log(`   Audio track settings:`, event.track.getSettings());
//       }
...
//       // Process any pending ICE candidates for this user
//       const pendingCandidates =
//         this.pendingIceCandidatesByUserId.get(fromUserId) || [];
//       if (pendingCandidates.length > 0) {
//         console.log(
//           `Processing ${pendingCandidates.length} pending ICE candidates for ${fromUserId}`,
//         );
...
//         event.track.enabled = true;
//         console.log(`   Video track settings:`, event.track.getSettings());
//       }
...
//     this.relayAttempted = new Set(); // Track relay attempts per user
//     this.connectionRetryCount = new Map(); // Track retry counts per user
//     this.iceErrorSuppressed = false; // Suppress ICE errors after successful connection
...
//     // Configuration with prioritized STUN/TURN servers
//     this.config = {
//       iceServers: [
//         // Prioritize Google STUN servers (most reliable)
//         { urls: "stun:stun.l.google.com:19302" },
//         { urls: "stun:stun1.l.google.com:19302" },
//         { urls: "stun:stun2.l.google.com:19302" },
//         { urls: "stun:stun3.l.google.com:19302" },
...
//         // Only log host candidates (local) at debug level, not errors
//         if (candidateType === "host") {
//           console.log(
//             `❄️ Sending local ICE candidate for ${userId}`,
//           );
//         } else {
//           // For relay/server reflexive candidates, log at debug level
//           console.debug(
//             `❄️ Sending ${candidateType} ICE candidate for ${userId}`,
//           );
//         }
...
//     pc.onicecandidateerror = (event) => {
//       // Suppress ICE errors after connection is established
//       const pc = this.peerConnections.get(userId);
//       if (pc && (pc.iceConnectionState === "connected" || pc.iceConnectionState === "completed")) {
//         // Connection is working, suppress errors
//         return;
//       }
      
//       // Only log as debug, not error, since these are expected
//       console.debug(`ℹ️ ICE candidate error for ${userId} (normal):`, {
//         errorCode: event.errorCode,
//         errorText: event.errorText,
...
//       console.log(`❄️ ICE state with ${userId}:`, state);
...
class WebRTCService {
  ...
  _createPeerConnection(userId, forceRelay = false) {
    ...
    pc.ontrack = (event) => {
      ...
      // Get the stream - it could be event.streams[0] or we need to create one
      ...
      if (!stream) {
        ...
        console.log(`Created new stream for track from ${userId}`);
      }
      ...
      // Store the stream and notify
      this.remoteStreams.set(userId, stream);
      
      // Important: Fire the callback with the stream
      if (this.onRemoteStream) {
        console.log(`📢 Firing onRemoteStream for ${userId}`);
        this.onRemoteStream(userId, stream);
      }
    };
