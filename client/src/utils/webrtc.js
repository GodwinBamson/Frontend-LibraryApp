...
class WebRTCService {
  ...
  initialize(userId, username, role) {
    ...
    setTimeout(() => {
      this.testTurnServers().then((relayWorking) => {
        console.log("TURN servers test result:", relayWorking ? "Working" : "Not working");
      });
    }, 2000);
  ...
  async retryWithRelay(userId) {
    ...
    if (this.localStream) {
      this.localStream.getTracks().forEach((track) => {
        console.log(`Adding ${track.kind} track to relay connection for ${userId}`);
        pc.addTrack(track, this.localStream);
      });
    } else {
  ...
  _createPeerConnection(userId, forceRelay = false) {
    ...
    console.log(`Creating peer connection for ${userId}${forceRelay ? ' (forcing relay)' : ''}`);
    ...
    pc.onicecandidate = (event) => {
      if (event.candidate) {
        ...
        console.log(`❄️ Sending ICE candidate for ${userId} [Type: ${candidateType}]`);
      ...
      switch(pc.iceConnectionState) {
        ...
        case "failed":
          ...
          if (!this.relayAttempted.has(userId) && retryCount < 3) {
            console.log(`Attempting relay connection for ${userId} (retry ${retryCount + 1}/3)`);
