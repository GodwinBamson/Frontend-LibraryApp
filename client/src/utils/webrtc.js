...
class WebRTCService {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        this.socket.once("room-joined", (data) => {
          ...
          // Setup audio monitoring after room joined
          this._setupAudioMonitoring();
  ...
  // Updated audio monitoring method with better context handling
  _setupAudioMonitoring() {
    if (!this.localStream || !window.AudioContext) return;
    ...
    try {
      ...
      // If context is suspended, try to resume after a short delay
      if (this.audioContext.state === 'suspended') {
        setTimeout(() => {
          if (this.audioContext && this.audioContext.state === 'suspended') {
            this.audioContext.resume().catch(err => {
              console.log('AudioContext resume delayed - waiting for user interaction');
            });
          }
        }, 1000);
      }
      ...
      const checkAudioLevel = () => {
        if (!this.localStream || !this.socket?.connected || !this.audioContext) return;
        ...
        try {
          ...
          console.error('Error checking audio level:', err);
        }
  ...
  // New method to ensure audio is enabled
  ensureAudioEnabled() {
    if (this.localStream) {
      ...
      // Also try to resume audio context if it exists
      if (this.audioContext && this.audioContext.state === 'suspended') {
