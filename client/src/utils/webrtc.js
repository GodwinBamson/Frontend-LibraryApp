...
class WebRTCService {
  ...
  _setupListeners() {
    ...
    this.socket.on("reconnect", () => {
      ...
      // If we were in a call, re-establish it
      if (this.callInProgress && this.currentRoom) {
        console.log("Re-establishing call after reconnect");
        this.socket.emit("join-room", {
          roomId: this.currentRoom,
    ...
    this.socket.on("user-joined", async (data) => {
      ...
      // Only create offer if we are in a call and this is the other participant
      if (this.callInProgress && data.userId !== this.currentUser?.userId) {
        console.log("User joined during active call, creating offer");
    ...
    this.socket.on("call-accepted", ({ from, roomId, callType }) => {
      ...
      if (this.currentRoom !== roomId) {
        ...
        // Ensure local stream is set up before joining room
        this._ensureLocalStream().then(() => {
          this.socket.emit("join-room", { roomId, callType });
          
          // Create offer for the other participant
          setTimeout(() => {
            if (from.userId) {
              this._createOffer(from.userId);
            }
          }, 1000);
        });
      }
      ...
      if (data.participants) {
        ...
        participants.forEach((p) => {
          ...
          if (targetUserId && targetUserId !== this.currentUser?.userId) {
            ...
            setTimeout(() => this._createOffer(targetUserId), 1000);
          }
        });
      }
    });
    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        this.socket.once("room-joined", (data) => {
          ...
          // Create offer for the other participant after joining room
          setTimeout(() => {
  ...
  async acceptCall(roomId, callType) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        const callerUserId = Array.from(this.pendingCalls.keys())[0];
        if (callerUserId) {
          console.log(`Accepting call from: ${callerUserId}`);
          this.socket.emit("accept-call", {
            from: callerUserId,
            roomId,
            callType,
        ...
        this.socket.once("room-joined", (data) => {
          ...
          // Create offer for the caller after joining room
          if (callerUserId) {
            setTimeout(() => {
              this._createOffer(callerUserId);
            }, 1000);
          }
