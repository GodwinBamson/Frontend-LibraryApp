...
export default function Books() {
  ...
  const handleRead = (pdfUrl, bookId, bookTitle) => {
    // Check if device is mobile
    const isMobile = /iPhone|iPad|iPod|Android/i.test(navigator.userAgent);
    
    if (isMobile) {
      // For mobile, use our custom mobile-friendly viewer
      const baseUrl = import.meta.env.VITE_API_URL || 'http://localhost:5000';
      window.open(`${baseUrl}/api/view-pdf/${bookId}`, '_blank');
    } else {
      // For desktop, use the modal PDF reader
  ...
  const getPdfUrl = (book) => {
    // If book has pdfUrl field (frontend expects), use that
    if (book.pdfUrl) return book.pdfUrl;
    // Otherwise use pdfFile from backend
    if (book.pdfFile) return book.pdfFile;
    return null;
  };
                    ...
                    <div className="flex flex-col gap-3">
                      ...
                      {pdfUrl && (
                        <div className="flex items-center justify-between bg-green-50 p-3 rounded-md border border-green-200">
                          ...
                          <button
                            onClick={() => handleRead(pdfUrl, book._id, book.title)}
                            className="inline-flex items-center px-3 py-1.5 bg-green-600 text-white text-xs font-medium rounded-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition-colors"
                          >
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
