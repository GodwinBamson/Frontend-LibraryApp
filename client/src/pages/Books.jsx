...
export default function Books() {
    ...
    try {
      ...
      response.data.forEach(book => {
        ...
        if (pdfUrl) {
          ...
          console.log(`   Is Cloudinary: ${pdfUrl.includes('cloudinary.com')}`);
        }
      });
    } catch (error) {
      console.error("Error fetching books:", error);
  ...
  const handleRead = (pdfUrl, bookTitle) => {
    ...
    if (!pdfUrl) {
      alert("PDF URL is not available for this book");
      return;
    }
    
    if (isMobile) {
      // For mobile: open in new tab with viewer options to disable download/print
      const viewerUrl = pdfUrl.includes('cloudinary.com') 
        ? `${pdfUrl}#toolbar=0&navpanes=0&scrollbar=0&view=FitH`
        : pdfUrl;
      
      console.log("📱 Opening in new tab:", viewerUrl);
      window.open(viewerUrl, '_blank');
    } else {
      // For desktop: use modal viewer
      setSelectedPdf({ url: pdfUrl, title: bookTitle });
    }
  };
                      ...
                      {pdfUrl && (
                        <div className="space-y-2">
                          ...
                          <div className="flex items-center justify-between bg-green-50 p-3 rounded-md border border-green-200">
                            ...
                            <button
                              onClick={() => handleRead(pdfUrl, book.title)}
                              className="inline-flex items-center px-3 py-1.5 bg-green-600 text-white text-xs font-medium rounded-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition-colors"
                            >
                              <svg
                                xmlns="http://www.w3.org/2000/svg"
                          ...
                          {/* Mobile download button - optional */}
                          {isMobile && (
                            <button
                              onClick={() => handleDownload(pdfUrl, book.title)}
                              className="w-full px-3 py-2 bg-purple-600 text-white text-xs font-medium rounded-md hover:bg-purple-700 transition-colors flex items-center justify-center"
