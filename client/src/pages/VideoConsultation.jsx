...
//   // Filter and sort messages with safe array checking
...
//         </span>
//         {transport !== 'unknown' && (
//           <span className="transport-badge text-gray-300">
//             {transport}
//           </span>
//         )}
//         {showReconnect && (
...
const VideoConsultation = () => {
  ...
  useEffect(() => {
    const handleUserInteraction = () => {
      ...
      WebRTCService.ensureAudioEnabled?.();
    };
  ...
  // Ensure audio is enabled when call starts or is accepted
  useEffect(() => {
    if (WebRTCService.currentRoom && (callType === 'audio' || callType === 'video')) {
      ...
      const timer = setTimeout(() => {
        ...
        // Force audio context to resume on user interaction
        const resumeAudio = () => {
          if (WebRTCService.localStream) {
  ...
  }, [WebRTCService.currentRoom, callType, isAudioEnabled]);
  ...
  }, [WebRTCService.currentRoom, isAudioEnabled]);
  ...
  const toggleAudio = () => {
    ...
    // If enabling audio, ensure it's working
    if (newState) {
      setTimeout(() => {
        WebRTCService.ensureAudioEnabled?.();
      }, 100);
  ...
  return (
    <>
      ...
      {audioLevel > 0.1 && WebRTCService.currentRoom && (
        <div className="audio-visualizer">
          {[...Array(5)].map((_, i) => (
            <div
              ...
              style={{
                height: `${Math.max(4, Math.min(20, audioLevel * 40))}px`,
