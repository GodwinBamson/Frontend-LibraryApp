...
const VideoConsultation = () => {
  ...
  const enableAudio = () => {
    ...
    Object.values(remoteStreams).forEach(stream => {
      stream.getAudioTracks().forEach(track => {
        track.enabled = true;
        if (track.muted) {
          console.log(`🔊 Unmuting audio track via user interaction`);
  ...
  useEffect(() => {
    ...
    Object.entries(remoteStreams).forEach(([userId, stream]) => {
      ...
      console.log(`User ${userId} audio tracks:`, audioTracks.length);
    ...
    Object.entries(remoteVideosRef.current).forEach(([userId, videoEl]) => {
      if (videoEl) {
        console.log(`Video element ${userId} muted:`, videoEl.muted);
        if (videoEl.muted) {
          console.log(`🔊 Unmuting video element for ${userId}`);
          videoEl.muted = false;
        }
        if (videoEl.volume === 0) {
          videoEl.volume = 1.0;
      ...
      if (videoEl && stream && stream.active) {
        if (!videoEl.srcObject || videoEl.srcObject.id !== stream.id) {
          ...
          stream.getAudioTracks().forEach(track => {
            track.enabled = true;
            if (track.muted) {
              console.log(`🔊 Unmuting audio track for ${userId} during force attach`);
              track.muted = false;
            }
          });
  ...
  // Periodic check for muted tracks
  useEffect(() => {
    if (Object.keys(remoteStreams).length === 0) return;
    ...
    const interval = setInterval(() => {
      ...
      Object.entries(remoteStreams).forEach(([userId, stream]) => {
        stream.getAudioTracks().forEach(track => {
          if (track.muted) {
            console.log(`🔊 Found muted track for ${userId}, unmuting...`);
            track.muted = false;
          }
          if (!track.enabled) {
            console.log(`🎤 Found disabled track for ${userId}, enabling...`);
            track.enabled = true;
          }
        });
      });
  ...
  return (
    <>
      ...
      {/* Audio Status Banner */}
      {Object.keys(remoteStreams).length > 0 && !Object.values(audioEnabled).some(Boolean) && (
        <div className="fixed top-16 left-1/2 transform -translate-x-1/2 bg-yellow-600 text-white px-6 py-3 rounded-full shadow-lg z-50 flex items-center space-x-3">
          <VolumeX size={20} />
          <span>Click anywhere to enable audio</span>
          <button
            onClick={enableAudio}
            className="ml-2 px-4 py-1 bg-white text-yellow-600 rounded-full text-sm font-semibold hover:bg-gray-100"
          >
            Enable Now
          </button>
        </div>
                    ...
                    return (
                      ...
                      >
                        <video
                              ...
                              if (stream && stream.active) {
                                if (!el.srcObject || el.srcObject.id !== stream.id) {
                                  ...
                                  stream.getAudioTracks().forEach(track => {
                                    track.enabled = true;
                                    if (track.muted) {
                                      console.log(`🔊 Unmuting audio track for ${userId} at attachment`);
                                    ...
                                    el.play()
                                      .then(() => {
                                        ...
                                        setTimeout(() => {
                                          ...
                                          // Check tracks - this is the most important part
                                          stream.getAudioTracks().forEach(track => {
                                            if (track.muted) {
                                              console.log(`🔊 Unmuting audio track for ${userId} after play`);
                                              track.muted = false;
                                            }
                                            if (!track.enabled) {
                                      ...
                                      .catch(err => {
                                        console.log(`⚠️ Play failed for ${userId}:`, err);
                                        setRemoteVideoReady(prev => ({ ...prev, [userId]: false }));
                                      });
                                  };
                          ...
                          onVolumeChange={() => {
                            const el = remoteVideosRef.current[userId];
                            if (el) {
                              console.log(`Volume changed for ${userId}:`, el.volume, el.muted);
                            }
                          }}
                        />
                        ...
                        <div className="audio-indicator">
                          {audioEnabled[userId] ? (
                            <>
                              <Volume2 size={14} className="audio-enabled" />
                              <span className="audio-enabled">Audio On</span>
                            </>
                          ) : (
                            <>
                              <VolumeX size={14} className="audio-disabled" />
                              <span className="audio-disabled">Audio Off</span>
                            </>
                          )}
                        </div>
