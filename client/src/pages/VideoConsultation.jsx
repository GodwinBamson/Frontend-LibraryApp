...
const VideoConsultation = () => {
  ...
  const fileInputRef = useRef();
  const healthCheckIntervalRef = useRef(null);
  const remoteStreamCheckIntervalRef = useRef(null);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (remoteStreamCheckIntervalRef.current) {
        clearInterval(remoteStreamCheckIntervalRef.current);
      }
      leaveCall();
      WebRTCService.disconnect();
      attachedStreamsRef.current.clear();
    };
  }, [user, addSystemMessage, scrollToBottom, showChat]);
  ...
  // Attach remote streams to video elements with retry mechanism
  useEffect(() => {
        ...
        if (videoEl && stream && stream.active) {
          ...
          if (videoEl.paused) {
            videoEl.play()
              ...
              .catch((err) => {
                console.log(`Play button shown for user ${userId}:`, err);
                const playBtn = document.getElementById(`play-btn-${userId}`);
                if (playBtn) playBtn.style.opacity = "1";
              });
          }
    ...
    remoteStreamCheckIntervalRef.current = setInterval(attachRemoteStreams, 1000);
