...
//         <div>
//           <h1 className="text-3xl font-bold text-gray-900">Book Management</h1>
//           <p className="text-gray-600 mt-2">Add, edit, or remove books from the library</p>
//         </div>
//         <button
...
//                 <input type="number" name="publishedYear" value={formData.publishedYear} onChange={handleChange} required min="1000" max={new Date().getFullYear()}
//                   className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500"/>
//               </div>
//               <div>
//                 <label className="block text-sm font-medium text-gray-700 mb-1">Total Copies *</label>
//                 <input type="number" name="totalCopies" value={formData.totalCopies} onChange={handleChange} required min="1"
//                   className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500"/>
//               </div>
//             </div>
...
//                     </p>
//                     <p className="text-xs text-gray-500">PDF files only (Max size: 50MB)</p>
//                   </div>
//                   <input type="file" name="pdfFile" accept=".pdf,application/pdf" onChange={handleChange} className="hidden"/>
//                 </label>
//               </div>

//               {formData.pdfFile && (
//                 <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded-md flex justify-between items-center">
//                   <span className="text-sm font-medium text-gray-700">{formData.pdfFile.name}</span>
//                   <span className="text-xs text-gray-500">{(formData.pdfFile.size / 1024 / 1024).toFixed(2)} MB</span>
//                 </div>
//               )}

//               {editingBook && editingBook.pdfFilename && !formData.pdfFile && (
//                 <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-md">
//                   <span className="text-sm text-gray-700">Current PDF: {editingBook.pdfFilename}</span>
...
//                 {uploading ? "Processing..." : editingBook ? "Update Book" : "Add Book"}
//               </button>
//             </div>
//           </form>
//         </div>
...
//                   </td>
//                   <td className="px-6 py-4">
//                     <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${book.totalCopies > 0 ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}>
//                       {book.totalCopies > 0 ? "Available" : "Out of stock"}
//                     </span>
//                   </td>
//                   <td className="px-6 py-4 flex space-x-2">
//                     {book.pdfFile ? (
//                       <>
//                         <button onClick={() => handlePdfView(book)} className="text-indigo-600 hover:text-indigo-900 text-sm">View</button>
//                         <button onClick={() => handleDownloadPdf(book)} className="text-green-600 hover:text-green-900 text-sm">Download</button>
//                       </>
//                     ) : (
//                       <span className="text-xs text-gray-400">No PDF</span>
//                     )}
//                   </td>
//                   <td className="px-6 py-4 flex space-x-2">
//                     <button onClick={() => handleEdit(book)} className="text-blue-600 hover:text-blue-900"><PencilIcon /></button>
//                     <button onClick={() => handleDelete(book._id)} className="text-red-600 hover:text-red-900"><TrashIcon /></button>
//                   </td>
//                 </tr>
//               )) : (
//                 <tr>
...
export default function AdminBooks() {
  ...
  const categories = [
    "Fiction", "Non-Fiction", "Science", "Technology",
    "History", "Biography", "Education", "Religion", "Art", "Business"
  ];
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.title || !formData.author || !formData.isbn || !formData.category || !formData.description) {
    ...
    try {
      ...
      for (let pair of formDataToSend.entries()) {
        console.log(pair[0] + ": " + (pair[0] === 'pdfFile' ? pair[1].name : pair[1]));
      }
            ...
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 bg-gray-50">
              ...
              {editingBook && editingBook.pdfFilename && !formData.pdfFile && (
                <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-md">
                  ...
                  <p className="text-xs text-gray-500 mt-1 ml-7">Upload a new file to replace the existing one</p>
                </div>
              )}
            </div>
                  ...
                  filteredBooks.map((book) => (
                    <tr key={book._id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4">
                        ...
                        <div className="text-sm text-gray-500">{book.category}</div>
                        {book.publisher && (
                          <div className="text-xs text-gray-400">{book.publisher} • {book.publishedYear}</div>
                        )}
                      </td>
                      <td className="px-6 py-4">
                  ...
                  <tr>
                    <td colSpan={5} className="px-6 py-12 text-center text-gray-500">
                      {searchTerm || selectedCategory ? (
                        <div>
                          <p className="text-lg mb-2">No books match your search criteria</p>
                          <button
                            onClick={() => {
                              setSearchTerm("");
                              setSelectedCategory("");
